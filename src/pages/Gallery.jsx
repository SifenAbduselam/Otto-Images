    ...
    <div className="bg-black text-white pt-28 px-6">
      ...
      <div className="flex flex-col gap-24">
        {categories.map((cat, index) => {
          const previewImage = imagesByCategory[cat][0]; // first image
          return (
            <motion.div
              key={cat}
            ...
            >
              {previewImage ? (
                ...
                <motion.div
                  className="w-full md:w-1/2 h-64 bg-gray-800 flex items-center justify-center text-gray-500 text-lg rounded-lg"
                  initial={{ opacity: 0 }}
              ...
              <motion.div
                ...
                transition={{ duration: 0.7 }}
              >
                <h2 className="text-3xl font-bold text-[#C8A35F] capitalize">{cat}</h2>
                <p className="text-white text-lg">
                  Explore stunning {cat} moments captured beautifully by our photographers.
                </p>
                <Link
                  to={`/gallery/${cat}`}
